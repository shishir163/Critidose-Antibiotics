 ...
 b:[
  ...
  {tbl:{head:['Item','Source'],
   rows:[
    ...
    ['Ceftazidime renal bands — corrected at the low end (CrCl 6–15 and under 5)','Product information; the app previously gave the wrong maintenance dose below CrCl 15'],
    ['Meropenem, cefepime, piperacillin-tazobactam, aztreonam renal bands','Cross-checked against product information and standard renal dosing charts — matched, no change needed'],
    ['Ampicillin renal bands — corrected at CrCl under 10','Standard renal dosing charts; the app previously kept the interval too short at severe impairment'],
    ['Ampicillin-sulbactam, cefoperazone-sulbactam, cefuroxime, cefazolin, cefotaxime, ertapenem','Cross-checked against product information — matched, no change needed'],
    ['Ciprofloxacin renal bands — corrected boundaries','Standard renal dosing charts; the mid and low bands started at the wrong CrCl'],
    ['Teicoplanin renal bands — corrected boundaries','Targocid product information; the app used the wrong CrCl cut-offs (80/30 instead of 60/40)'],
    ['Levofloxacin, moxifloxacin (no renal adjustment), tigecycline (no renal adjustment, hepatic dose confirmed), doxycycline, minocycline','Cross-checked against product information — matched, no change needed'],
    ['Linezolid, daptomycin, polymyxin B','Cross-checked against product information and pharmacokinetic studies — matched, no change needed'],
    ['Fluconazole, voriconazole (renal and hepatic), caspofungin (renal and hepatic)','Cross-checked against product information — matched, no change needed'],
    ['Acyclovir, oseltamivir','Cross-checked against standard renal dosing charts — matched, no change needed'],
    ['Metronidazole, clindamycin, azithromycin, rifampicin, liposomal amphotericin B (all "no renal adjustment")','Cross-checked — matched, no change needed'],
    ['Aztreonam-avibactam renal ladder and loading dose','Emblaveo prescribing information and EMA summary of product characteristics'],
    ['Molecular panel suggestions','IDSA Guidance on the Treatment of Antimicrobial-Resistant Gram-Negative Infections (2024, updated 2026); FilmArray PN validation studies for the predictive-value figures'],
    ['Respiratory panel buckets','IDSA/ATS community-acquired pneumonia 2019 and hospital-acquired pneumonia 2016. The colonisation judgements reflect standard critical care practice rather than a single citable table'],
    ['Imipenem-cilastatin renal bands — rebuilt','Primaxin (imipenem-cilastatin) prescribing information; the app extended the interval when the label reduces the dose instead — the whole ladder was restructured'],
    ['Flucloxacillin — capped the severe-impairment dose at the label maximum','Multiple national product information sheets (Ireland, New Zealand, South Africa) agree: max 1 g q8–12h in renal failure'],
    ['Benzylpenicillin renal bands — corrected cut-offs','A published renal-impairment dosing table; the app changed dose at the wrong CrCl and with the wrong shape'],
    ['Gentamicin extended-interval bands','Cross-checked against the Hartford nomogram and multiple hospital protocols — matched, no change needed'],
    ['Amikacin — tightened the intermediate bands to reduce the dose as well as extend the interval','Hartford-style nomogram sources'],
    ['Micafungin — hepatic note broadened to match the FDA label (no adjustment even in severe impairment)','FDA prescribing information'],
    ['Fosfomycin IV — added a caveat rather than a fix','No single reliable renal table was found; the IV formulation genuinely differs by country (European disodium salt vs the US-approved Contepo), so the existing ladder is left as a starting point with that caveat attached'],
  ...
  {h:'Not yet checked'},
  {p:'Still not pinned to an exact primary-source table: amoxicillin-clavulanate IV, and cotrimoxazole (both are pattern-matched against several partial sources rather than one clean table — the direction and rough magnitude are right, the exact cut-offs may not be).'},
  {h:'Overall'},
  {p:'All 48 antibiotics in this app have now been checked against at least one product information sheet, consensus guideline, or standard renal dosing reference. Eleven corrections were made along the way: colistimethate, vancomycin, ceftazidime, ampicillin, ciprofloxacin, teicoplanin, imipenem-cilastatin, flucloxacillin, benzylpenicillin, amikacin and micafungin\u2019s hepatic note. Two entries — amoxicillin-clavulanate and cotrimoxazole — are reasonable but not tied to one authoritative table, and fosfomycin IV carries a country-specific caveat rather than a single fixed answer. Everything else has matched its source without needing a change.'},
...
{n:'Reading a molecular resistance panel', al:'biofire filmarray PCR panel gene NDM OXA CTX-M interpretation', k:'resistance',
 sub:'What it can tell you, and the one thing it cannot',
 b:[
  {p:'A multiplex PCR panel such as the BioFire FilmArray gives you organisms and resistance genes in about an hour, against five days for a culture. The Molecular panel tile turns a result into suggestions. This page explains what those suggestions rest on.'},
  {h:'The negative is stronger than the positive'},
  {p:'Reported sensitivity and negative predictive value for the resistance targets approach 100%. A gene reported as Not Detected is therefore a reasonable basis for stopping a drug \u2014 mecA not detected is the single most useful result on the report, because it lets you stop empiric vancomycin.'},
  {p:'Positive predictive value is another matter. One validation series put it between 25 and 41% against culture; others have found much better agreement, particularly for mecA and CTX-M. A gene can be present and not expressed. Escalating on a positive is a bridge, not a conclusion.'},
  {h:'It does not link a gene to an organism'},
  {p:'This is the limitation that matters most in a ventilated ICU patient, where three or four Gram-negatives are often reported together. If NDM sits in the Acinetobacter, the regimen is sulbactam-based. If it sits in the Klebsiella, the regimen is aztreonam-avibactam. The panel cannot distinguish those two situations, and they are not treated the same way.'},
  {h:'It does not distinguish colonisation from infection'},
  {p:'Neither does a culture. A tracheal aspirate is a colonised sample in most ventilated patients. Treat the patient in front of you, not the report.'},
  {h:'Gene by gene'},
  {tbl:{head:['Gene','Means','Still works'],
   rows:[
    ['CTX-M','ESBL \u2014 cephalosporins and aztreonam fail','Carbapenems. Piperacillin-tazobactam is unreliable in bacteraemia'],
    ['KPC','Class A carbapenemase','Ceftazidime-avibactam'],
    ['OXA-48-like','Class D carbapenemase','Ceftazidime-avibactam'],
    ['NDM, VIM, IMP','Metallo-enzyme \u2014 every beta-lactam fails, including ceftazidime-avibactam','Aztreonam-avibactam; or ceftazidime-avibactam plus aztreonam together; cefiderocol where stocked'],
    ['mecA/C and MREJ','MRSA, when S. aureus is also detected','Vancomycin, linezolid']]}},
  {note:'CTX-M is the only ESBL target on the panel. TEM and SHV enzymes are missed entirely, so a negative CTX-M does not exclude ESBL.'},
  {h:'Where the suggestions come from'},
  {p:'IDSA Guidance on the Treatment of Antimicrobial-Resistant Gram-Negative Infections, filtered through what your unit stocks. You can change that stock list from the panel page \u2014 it decides whether a preferred agent is offered or noted as unavailable.'}
 ]},
